-- Production hardening: remove PL/pgSQL output-column ambiguity from RPCs.
-- This migration is forward-only and intentionally preserves signatures,
-- SECURITY DEFINER boundaries, empty search_path, privileges, and retry budgets.
...
begin
  ...
  insert into public.captures as capture_row (
    workspace_id,
    project_id,
    title,
    raw_text,
  ...
  returning capture_row.* into v_capture;
  ...
  if v_job.id is null then
    insert into public.processing_jobs as processing_job_row (
      workspace_id,
      capture_id,
      status,
      job_type,
...
begin
  ...
  loop
    ...
    select node_map.id into v_source_node_id
    from pg_temp.analysis_node_map as node_map
    where node_map.client_id = v_edge->>'sourceClientNodeId';

    select node_map.id into v_target_node_id
    from pg_temp.analysis_node_map as node_map
    where node_map.client_id = v_edge->>'targetClientNodeId';
